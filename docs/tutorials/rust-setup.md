...
!!! note "Citations"
    Instructions in this section is reused+edited from the 423 [MkDocs tutorial](https://comp423-25s.github.io/resources/git/ch2-git-fundamental-subcommands/#configure-your-name-and-email) by Kris Jordan.  
...
* ```image```: The Docker image to use, in this case, is the latest version of a Rust environemnt. [Microsoft has a collection of base images for many programming language environemnts, but you can also customize your own by providing the path to a Docker file.](https://hub.docker.com/r/microsoft/vscode-devcontainers)
...
!!! quote "What is rust-analyzer?"
     Quoted from [GitHub](https://rust-analyzer.github.io/), the rust-analyzer extension provides useful features like comletion and goto definition for many code editors.
``` py
{
    "name": "Rust Tutorial",
...
!!! question "What does this mean?"
    * ``` cargo new hello_comp423 --vcs none
    ```: ```cargo new``` creates a new Cargo package that has the directory and project named hello_world, and Cargo creates its files in a directory of the same name. The flag ```--vcs none``` stands for Version Control System none which doesn't create a new git repository automatically on your behalf.
    * ``` cd hello_comp423
    ```: This goes into your hello_world directory and files. You'll see that Cargo has two files and one directory: a Cargo.toml file and a src directory with a main.rs file inside.
Open ```src/main.rs```. You'll see something like this:
``` py
fn main() {
    println!("Hello, world!");
}
...
```
This command creates an executable file in target/debug/hello_comp423 instead of your current directory. Since the defauly build is a debug build, Cargo puts the binary in a directory named debug. You can run the executable with the command
```
...
!!! question "How is this different from ```build```?"
    ...
    * ```cargo run``` combines the function of cargoo build and running the compiled binary. It'll compile the project if needed and then immediately run the resulting executable. You'll want to use this if you want to test or debug your program and see its output quickly.
